...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    const mappedItems = galleryItems.map(item => {
      if (item.image && item.text && !item.id) {
        const text = item.text.length > 20 ? item.text.substring(0, 17) + "..." : item.text;
        return { ...item, text };
      }
      ...
      const image = item.poster_path
        ? `https://image.tmdb.org/t/p/w342${item.poster_path}?cors=true`
    ...
    this.mediasImages = mappedItems.concat(mappedItems);
  ...
  onTouchDown(e) {
    ...
    this.start = clientX;
    this.clickStart = { x: clientX, y: clientY, time: Date.now() };
  }
  onTouchMove(e) {
  ...
  onTouchUp(e) {
    ...
    if (this.clickStart && e) {
      const upEvent = e.changedTouches ? e.changedTouches[0] : e;
      ...
      if (clientX !== undefined && clientY !== undefined) {
        ...
        const timeDiff = Date.now() - this.clickStart.time;
        
        if (timeDiff < 250 && dist < 8) {
  ...
  handleCanvasClick(clientX, clientY) {
    if (!this.renderer || !this.renderer.gl || !this.renderer.gl.canvas) return;
    ...
    const viewX = ndcX * (this.viewport.width / 2);
    const viewY = ndcY * (this.viewport.height / 2);
    
    const clickedMedia = this.medias.find(media => {
      const halfWidth = media.plane.scale.x / 2;
      const halfHeight = media.plane.scale.y / 2;
      const dx = viewX - media.plane.position.x;
      const dy = viewY - media.plane.position.y;
      return Math.abs(dx) <= halfWidth && Math.abs(dy) <= halfHeight;
    });
    
    if (clickedMedia && clickedMedia.data && this.onItemClick) {
